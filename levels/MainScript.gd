...
func NewDay():
	...
	day += 1
	print (day)
	$resources/VBoxContainer2/daylbl.text = "Day " + str(day)
	resource_update()
	income += incomerate
	$resources/VBoxContainer2/incomelbl.text = "Income: $" + str(income)
	disaster_roll()
	disastertime -= 1
	...
	if disaster == null: # First checks if you already have a disaster
		...
		if day < 5 and day > 1 or day == 5: # Second checks what day it is to make sure disaster is chosen on or before day 5
			...
			if disasterchance >= 0.33: # 33 percent chance on day 2 to 4 for the disaster to be chosen
				...
				print (disaster)
				$resources/VBoxContainer2/disasterlbl.text = "Current Disaster: " + str(disaster) # This should be hidden from the player and events will reveal what disaster it is
